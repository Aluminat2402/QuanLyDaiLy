...
namespace QuanLyDaiLy.ViewModels.PhieuXuatViewModels
{
        ...
        IRecipient<DataReloadMessage>
    {
        ...
        public void Receive(SearchCompletedMessage<PhieuXuat> message)
        {
            var searchResults = message.Value;

            if (searchResults.Count > 0)
            {
                FilteredPhieuXuats = searchResults;
                TotalPages = (int)Math.Ceiling((double)FilteredPhieuXuats.Count / ItemsPerPage);
                CurrentPage = "1";
                _ = UpdatePagination();
            }
            else
        ...
        [ObservableProperty]
        private ObservableCollection<PhieuXuat> _filteredPhieuXuats = [];
        ...
        private async Task LoadDataAsync()
        {
            try
            {
                FilteredPhieuXuats.Clear();
                var list = await _phieuXuatService.GetPhieuXuatPage(0);
                DanhSachPhieuXuat = [.. list];

                TotalPages = await _phieuXuatService.GetTotalPages();
            ...
            {
                MessageBox.Show($"Lỗi khi tải dữ liệu phiếu xuất: {ex.Message}", "Lỗi", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        {
            if (FilteredPhieuXuats.Count == 0)
            {
                // Load from service when no filter is applied
                var items = await _phieuXuatService.GetPhieuXuatPage(currentPage - 1);
                DanhSachPhieuXuat = [.. items];
            }
            else
            {
                // Apply pagination to filtered results in memory
                int skip = (currentPage - 1) * ItemsPerPage;
                var pagedItems = FilteredPhieuXuats.Skip(skip).Take(ItemsPerPage);
                DanhSachPhieuXuat = [.. pagedItems];
            }
        }
        #endregion
    }
